...
class LLMMaxIterationsExceeded(Exception):
    """Raised when the conversation loop exceeds the maximum number of iterations."""
...
class LLMLoopConfig(BaseModel):
    ...
    max_iterations: int = Field(default=10, ge=1, description="Maximum number of loop iterations")
    parallel_tool_execution: bool = Field(default=True, description="Execute tools in parallel when possible")
...
class LLMConversationLoop:
    ...
    async def run(
        ...
        Raises LLMMaxIterationsExceeded if the loop exceeds max_iterations.
        """
